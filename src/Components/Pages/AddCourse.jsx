// src/Components/Pages/AddCourse.jsx
import { useState } from "react";
import { useNavigate } from "react-router-dom";

function AddCourse({ courses, setCourses }) {
  const navigate = useNavigate();
  const [form, setForm] = useState({ code: "", name: "", className: "", teacher: "" });

  const handleChange = (e) => setForm({ ...form, [e.target.name]: e.target.value });

  const addCourse = () => {
    if (!form.code || !form.name || !form.className || !form.teacher) return alert("All fields required");
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <input type="text" name="teacher" placeholder="Teacher" value={form.teacher} onChange={handleChange} />
